...
class TestRunner {
  ...
  async runAll(outputElement = null) {
    ...
    for (const test of testsToRun) {
      if (test.skip) {
        ...
        this.log(`[SKIP] ${test.name}`, outputElement, "skip");
        continue;
      }
      ...
      try {
        ...
        this.results.passed++;
        this.log(`[PASS] ${test.name}`, outputElement, "pass");
      } catch (error) {
        this.results.failed++;
        this.log(`[FAIL] ${test.name}`, outputElement, "fail");
  ...
  printSummary(outputElement) {
    ...
    this.log("\n----------------------------------------", outputElement);
    this.log("Summary", outputElement, "summary");
    ...
    this.log(`Time:    ${duration}ms`, outputElement);

    const successRate =
      this.results.total > 0
        ? ((this.results.passed / this.results.total) * 100).toFixed(1)
        : 0;
    this.log(`Success: ${successRate}%`, outputElement);
  }
...
class Assert {
  ...
  static assertNotEqual(actual, expected, message = "") {
    if (actual === expected) {
      throw new Error(
        message ||
          `Expected values to be different but both are ${JSON.stringify(
            actual
          )}`
      );
    }
  ...
  static assertTrue(value, message = "") {
    if (value !== true) {
      throw new Error(
        message || `Expected true but got ${JSON.stringify(value)}`
      );
    }
  }
  ...
  static assertFalse(value, message = "") {
    if (value !== false) {
      throw new Error(
        message || `Expected false but got ${JSON.stringify(value)}`
      );
    }
  }
