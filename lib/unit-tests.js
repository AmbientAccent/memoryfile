...
async function registerUnitTests(runner) {
  ...
  // ==========================================
  // Save Dialog Handling Tests
  // ==========================================

  runner.describe("Save Dialog Handling", () => {
    runner.test("should report cancellation from file picker", async () => {
      const original = window.showSaveFilePicker;
      window.showSaveFilePicker = async () => {
        throw new DOMException("User cancelled", "AbortError");
      };

      const result = await HTMLSQLiteFusion.saveHtmlFile(
