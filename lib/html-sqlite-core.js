...
class MemoryFile {
  ...
  /**
   * Save raw HTML content with consistent cancel handling.
   * This avoids apps dealing with AbortError directly.
   */
  static async saveHtmlFile(htmlContent, filename = "app.html") {
    ...
    // iOS WebKit: prefer Share if available
    if (capabilities.browser.isIOSAnyBrowser && capabilities.share) {
      try {
        const file = new File([htmlContent], filename, {
          type: "text/html",
