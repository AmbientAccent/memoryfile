...
async function registerPerformanceTests(runner) {
  ...
  runner.describe("Insert Performance", () => {
    runner.test("benchmark: single inserts (100 rows)", async () => {
      ...
      const count = fusion.exec("SELECT COUNT(*) as count FROM test");
      Assert.assertGreaterThan(count[0].count, 0);
  ...
  runner.describe("Query Performance", () => {
    ...
    runner.test(
      ...
      async () => {
        ...
        // Verify JOIN returns expected data structure
        const verifyResult = fusion.exec(`
          SELECT u.name, p.title, COUNT(c.id) as comment_count
          FROM users u
          JOIN posts p ON u.id = p.user_id
          LEFT JOIN comments c ON p.id = c.post_id
          ...
          LIMIT 1
        `);
        Assert.assertEqual(verifyResult.length, 1);
        Assert.assertDefined(verifyResult[0].name);
        Assert.assertDefined(verifyResult[0].title);
        Assert.assertDefined(verifyResult[0].comment_count);
  ...
  runner.describe("Memory Performance", () => {
    runner.test(
      "benchmark: database size scales with data",
      async () => {
        // This test verifies database size grows predictably with data
        // It uses getDatabaseSize() which is cross-browser compatible
        const fusion = new HTMLSQLiteFusion();
        await fusion.createDatabase();
        ...
        const sizeAfterInsert = fusion.getDatabaseSize();

        console.log(`Size after create: ${sizeAfterCreate} bytes`);
        console.log(`Size after 1000 rows: ${sizeAfterInsert} bytes`);
        console.log(`Size per row: ~${Math.round((sizeAfterInsert - sizeAfterCreate) / 1000)} bytes`);

        // Verify database grew with data
        Assert.assertGreaterThan(sizeAfterInsert, sizeAfterCreate);
        
        // Each row should add at least some bytes (id + 100 char string)
        // Expect at least 50 bytes per row on average
        Assert.assertGreaterThan(sizeAfterInsert - sizeAfterCreate, 50 * 1000);
    ...
    runner.test(
      ...
      async () => {
        ...
        for (let i = 1; i < results.length; i++) {
          const prevTime = results[i - 1].time;
          const currTime = results[i].time;
          const sizeRatio = results[i].size / results[i - 1].size;

          // Guard against division by zero or very small numbers
          if (prevTime < 1) {
            console.log(
              `Size increased ${sizeRatio}x, previous time too small to compare (${prevTime.toFixed(
                2
              )}ms)`
            );
            continue;
  ...
  runner.describe("Real-World Performance", () => {
    ...
    runner.test("benchmark: LIKE pattern search", async () => {
      const fusion = new HTMLSQLiteFusion();
      await fusion.createDatabase();
