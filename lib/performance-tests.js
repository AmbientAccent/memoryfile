...
async function registerPerformanceTests(runner) {
  ...
  runner.describe('Insert Performance', () => {
        ...
        () => {
          for (let i = 0; i < 100; i++) {
            fusion.run('INSERT INTO test VALUES (?, ?, ?)', 
              [i, TestDataGenerator.randomString(), TestDataGenerator.randomEmail()]);
          }
        },
        5
      );
    ...
    runner.test('benchmark: bulk insert (10000 rows)', async () => {
      const fusion = new HTMLSQLiteFusion();
      await fusion.createDatabase();
  ...
  runner.describe('Query Performance', () => {
    ...
    runner.test('benchmark: indexed vs non-indexed search', async () => {
      const fusion = new HTMLSQLiteFusion();
      await fusion.createDatabase();
      ...
      const withoutIndex = await PerformanceTest.measure(
        ...
        () => {
          fusion.exec('SELECT * FROM test WHERE email = ?', ['user2500@example.com']);
        },
        10
      );
    ...
    runner.test('benchmark: complex JOIN query', async () => {
      const fusion = new HTMLSQLiteFusion();
      await fusion.createDatabase();
      ...
      for (let i = 1; i <= 100; i++) {
        ...
        for (let j = 1; j <= 5; j++) {
          ...
          fusion.run('INSERT INTO posts VALUES (?, ?, ?)', [postId, i, `Post${postId}`]);
          for (let k = 1; k <= 3; k++) {
            const commentId = (postId - 1) * 3 + k;
            fusion.run('INSERT INTO comments VALUES (?, ?, ?)', [commentId, postId, `Comment${commentId}`]);
          }
        }
      }
      fusion.run('COMMIT');
  ...
  runner.describe('Update Performance', () => {
    runner.test('benchmark: single row updates', async () => {
      ...
      const result = await PerformanceTest.measure(
        ...
        () => {
          ...
          fusion.run('UPDATE test SET value = ? WHERE id = ?', ['updated', randomId]);
        },
        100
      );
    ...
    runner.test('benchmark: bulk update', async () => {
      ...
      const result = await PerformanceTest.measure(
        'Update 500 rows',
        () => {
          fusion.run('UPDATE test SET value = ? WHERE category = ?', ['updated', 'even']);
        },
        10
      );
  ...
  runner.describe('Export/Import Performance', () => {
    ...
    runner.test('benchmark: export medium database', async () => {
      const fusion = new HTMLSQLiteFusion();
      await fusion.createDatabase();
      fusion.run('CREATE TABLE test (id INTEGER, value TEXT)');
  ...
  runner.describe('Memory Performance', () => {
    runner.test('benchmark: memory usage with large dataset', async () => {
      ...
      for (let i = 0; i < 10000; i++) {
        fusion.run('INSERT INTO test VALUES (?, ?)', 
          [i, TestDataGenerator.randomString(100)]);
      }
      fusion.run('COMMIT');
      ...
      console.log(`Database size: ${(fusion.getDatabaseSize() / 1024 / 1024).toFixed(2)} MB`);
      ...
      const memoryFreed = (afterInsertMemory - afterCloseMemory) / 1024 / 1024;
  ...
  runner.describe('Scalability', () => {
    runner.test('benchmark: performance with increasing data size', async () => {
      ...
      for (let i = 1; i < results.length; i++) {
        ...
        console.log(`Size increased ${sizeRatio}x, time increased ${ratio.toFixed(2)}x`);
  ...
  runner.describe('Real-World Performance', () => {
    ...
    runner.test('benchmark: note search performance', async () => {
      ...
      fusion.run('CREATE VIRTUAL TABLE notes USING fts5(title, content)');
      
      // Insert 100 notes
      fusion.run('BEGIN TRANSACTION');
      for (let i = 0; i < 100; i++) {
        fusion.run('INSERT INTO notes VALUES (?, ?)', 
          [`Note ${i}`, TestDataGenerator.randomString(200)]);
      }
      fusion.run('COMMIT');
      
      const result = await PerformanceTest.measure(
        'Full-text search in 100 notes',
        () => {
          fusion.exec('SELECT * FROM notes WHERE notes MATCH ?', ['Note']);
        },
        20
      );
