...
async function registerIntegrationTests(runner) {
  ...
  runner.describe('Complete Workflow', () => {
    runner.test('should complete full CRUD cycle', async () => {
      ...
      // CREATE
      fusion.run('CREATE TABLE todos (id INTEGER PRIMARY KEY, task TEXT, done INTEGER)');
      
      // INSERT
      fusion.run('INSERT INTO todos (task, done) VALUES (?, ?)', ['Buy milk', 0]);
      fusion.run('INSERT INTO todos (task, done) VALUES (?, ?)', ['Walk dog', 0]);
    ...
    runner.test('should handle database creation with initial data', async () => {
      const fusion = new HTMLSQLiteFusion();
  ...
  runner.describe('Save and Load Cycle', () => {
    runner.test('should preserve data through export/import cycle', async () => {
  ...
  runner.describe('Multi-Table Operations', () => {
    ...
    runner.test('should handle complex multi-table transactions', async () => {
      ...
      fusion.run('INSERT INTO posts (user_id, title, content) VALUES (?, ?, ?)',
  ...
  runner.describe('Performance', () => {
    ...
    runner.test('should query indexed data efficiently', async () => {
      ...
      fusion.run('BEGIN TRANSACTION');
      for (let i = 0; i < 1000; i++) {
        fusion.run('INSERT INTO users VALUES (?, ?)', [i, `user${i}@example.com`]);
      }
      fusion.run('COMMIT');
      
      // Query should be fast with index
      const start = performance.now();
      const result = fusion.exec('SELECT * FROM users WHERE email = ?', ['user500@example.com']);
      const duration = performance.now() - start;
  ...
  runner.describe('Real-World Scenarios', () => {
    ...
    runner.test('Note Taking: Full-text search', async () => {
      ...
      // Add notes
      fusion.run('INSERT INTO notes_fts VALUES (?, ?)', 
        ['Shopping List', 'Buy milk, eggs, bread']);
      fusion.run('INSERT INTO notes_fts VALUES (?, ?)', 
        ['Meeting Notes', 'Discuss project timeline and milestones']);
      fusion.run('INSERT INTO notes_fts VALUES (?, ?)', 
        ['Recipe', 'Chocolate chip cookies with milk']);
      
      // Search for "milk"
      const results = fusion.exec(`
        SELECT * FROM notes_fts WHERE notes_fts MATCH 'milk'
      `);
      
      Assert.assertEqual(results.length, 2);
      Assert.assertContains(results[0].title, 'Shopping');
    ...
    runner.test('Contact Manager: Complex queries', async () => {
      ...
      for (const contact of contacts) {
        fusion.run('INSERT INTO contacts (name, email, phone, category, favorite) VALUES (?, ?, ?, ?, ?)', contact);
      }
      fusion.run('COMMIT');
      
      // Get favorites
      const favorites = fusion.exec('SELECT COUNT(*) as count FROM contacts WHERE favorite = 1');
      Assert.assertEqual(favorites[0].count, 2);
      
      // Get work contacts
      const work = fusion.exec('SELECT * FROM contacts WHERE category = ? ORDER BY name', ['Work']);
      Assert.assertEqual(work.length, 2);
      
      // Search by name
      const search = fusion.exec('SELECT * FROM contacts WHERE name LIKE ?', ['%Brown%']);
  ...
  runner.describe('Edge Cases', () => {
    runner.test('should handle empty database', async () => {
      ...
      Assert.assertGreaterThan(size, 0); // Even empty DB has header
    ...
    runner.test('should handle special characters in data', async () => {
      ...
      const specialChars = 'Hello "World" \'Single\' <tags> & symbols 日本語 emoji 🎉';
