...
async function registerIntegrationTests(runner) {
  ...
  runner.describe("Real-World Scenarios", () => {
    ...
    runner.test("Note Taking: LIKE-based text search", async () => {
      const fusion = new HTMLSQLiteFusion();
      await fusion.createDatabase();
      ...
      // Search for "milk" using LIKE pattern matching
  ...
  runner.describe("Storage Persistence", () => {
    runner.test("should persist database to IndexedDB", async () => {
      const fusion = new HTMLSQLiteFusion({
        encrypted: false,
        persistToStorage: true,
        storageKey: "test-persist-1",
      });
      await fusion.createDatabase();

      fusion.run("CREATE TABLE notes (id INTEGER PRIMARY KEY, text TEXT)");
      fusion.run("INSERT INTO notes VALUES (1, 'Test note')");

      // Save to IndexedDB
      const result = await fusion.save();
      Assert.assertTrue(result.success);
      Assert.assertEqual(result.method, "storage");
      ...
      await fusion2.clearStorage();
    });

    runner.test("should load database from IndexedDB", async () => {
      const storageKey = "test-persist-2";
      ...
      await fusion1.createDatabase();
      fusion1.run("CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT)");
      fusion1.run("INSERT INTO items VALUES (1, 'Item One')");
      fusion1.run("INSERT INTO items VALUES (2, 'Item Two')");
      await fusion1.save();
      fusion1.close();
      ...
      // Load from storage
      const stored = await fusion2.loadFromStorage();
      Assert.assertNotNull(stored);
      Assert.assertNotNull(stored.data);

      // Verify data
      fusion2.db = new fusion2.sqlJS.Database(stored.data);
      const result = fusion2.exec("SELECT * FROM items ORDER BY id");
      Assert.assertEqual(result.length, 2);
      Assert.assertEqual(result[0].name, "Item One");
      Assert.assertEqual(result[1].name, "Item Two");
    ...
    runner.test("should check for stored data", async () => {
      const storageKey = "test-persist-3";
      ...
      // Should have no stored data initially
      let stored = await fusion.hasStoredData();
      ...
      // Should have stored data now
      stored = await fusion.hasStoredData();
      Assert.assertNotNull(stored);
      Assert.assertTrue(stored.exists);
      Assert.assertNotNull(stored.timestamp);
    ...
    runner.test("should clear stored data", async () => {
      const storageKey = "test-persist-4";
      ...
      // Clear storage
      const cleared = await fusion.clearStorage();
    ...
    runner.test("should track if loaded from storage", async () => {
      const storageKey = "test-persist-5";

      // Create fresh instance - should not be from storage
      const fusion1 = new HTMLSQLiteFusion({
        encrypted: false,
        persistToStorage: true,
        storageKey: storageKey,
      ...
      await fusion1.createDatabase();
      Assert.assertFalse(fusion1.wasLoadedFromStorage());

      fusion1.run("CREATE TABLE test (id INTEGER)");
      await fusion1.save();
      fusion1.close();
      ...
      // Manually load from storage to verify
      const stored = await fusion2.loadFromStorage();
      if (stored) {
        fusion2.db = new fusion2.sqlJS.Database(stored.data);
        fusion2.loadedFromStorage = true;
      }

      Assert.assertTrue(fusion2.wasLoadedFromStorage());
    ...
    runner.test("should handle storage key generation from URL", async () => {
      const fusion = new HTMLSQLiteFusion({
        encrypted: false,
        persistToStorage: true,
        // No custom storageKey - should auto-generate
      });

      const key = fusion.getStorageKey();
      Assert.assertContains(key, "memoryfile:");
      Assert.assertContains(key, window.location.pathname);

      fusion.close();
    });

    runner.test("should persist to storage via save() method", async () => {
      const storageKey = "test-persist-6";
      ...
      await fusion.createDatabase();
      fusion.run("CREATE TABLE data (value TEXT)");
      fusion.run("INSERT INTO data VALUES ('important')");

      // save() persists to storage when persistToStorage is enabled
      const result = await fusion.save();
      Assert.assertTrue(result.success);
      Assert.assertEqual(result.method, "storage");

      // Verify it's in storage
      const stored = await fusion.hasStoredData();
      Assert.assertNotNull(stored);
  ...
  runner.describe("Edge Cases", () => {
    runner.test("should handle empty database", async () => {
      ...
      // sql.js in-memory databases return 0 bytes for truly empty DBs
      // This is expected behavior - the size only grows after modifications
      const emptySize = fusion.getDatabaseSize();
      Assert.assertEqual(emptySize, 0);

      // After any modification, database has content
      fusion.run("CREATE TABLE test (id INTEGER)");
      const sizeAfterCreate = fusion.getDatabaseSize();
      Assert.assertGreaterThan(sizeAfterCreate, 0);
