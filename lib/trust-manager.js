...
class TrustManager {
  constructor(config = {}) {
    this.config = {
      ...
      excludeSelectors: config.excludeSelectors || ['#trust-badge', '.trust-modal'],
  ...
  extractHashFromFilename(filename) {
    // Match: basename.{hash}.html where hash is 8-64 hex chars
  ...
  getBadgeContent(result) {
    const encryptionDetails = this.config.showEncryption && this.encryptionStatus ? this.getEncryptionDetails() : '';
    ...
    const statusMap = {
      'VERIFIED': {
        ...
        getDetails: (r) => `
          ...
          <div class="trust-detail-item">
            ...
            <code class="trust-hash" title="${r.fullHash || r.hash}">${r.hash}</code>
            <button class="trust-copy" data-copy="${r.fullHash || r.hash}" title="Copy full hash">📋</button>
          </div>
          <div class="trust-detail-item">
            <strong>Filename:</strong><br>
            <small>${this.escapeHtml(r.filename)}</small>
          </div>
          ${this.config.educationMode ? '<button class="trust-learn-more">Learn about verification →</button>' : ''}
        `
      },
      'SIGNED': {
        icon: '🔒',
        ...
        getDetails: (r) => `
          ...
          ${this.config.educationMode ? '<button class="trust-learn-more">Learn about signatures →</button>' : ''}
        `
      },
      'TIMESTAMPED': {
        icon: '⏰',
        ...
        getDetails: (r) => `
          ...
          ${this.config.educationMode ? '<button class="trust-learn-more">Learn about timestamping →</button>' : ''}
        `
      },
      'UNVERIFIED': {
        icon: '⚠',
      ...
      'TAMPERED': {
        ...
        getDetails: (r) => `
          ...
          ${this.config.educationMode ? '<button class="trust-learn-more">Why does this matter? →</button>' : ''}
        `
      }
    };
        ...
        <div class="trust-detail-item trust-encryption-status">
          ...
          ${metadata ? `
            <div style="margin-top: 8px; font-size: 12px; color: #7f8c8d;">
              ...
              <div>Key Derivation: ${metadata.kdf} (${metadata.iterations.toLocaleString()} iterations)</div>
            </div>
          ` : ''}
        </div>
      `;
