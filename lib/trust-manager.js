...
class TrustManager {
  ...
  normalizeHTML(html) {
    ...
    // Remove excluded elements using DOM parsing for accuracy.
    if (typeof DOMParser !== "undefined") {
      try {
        ...
        normalized = doc.documentElement.outerHTML;
      } catch (error) {
        // Fall back to regex removal if parsing fails.
      }
    }

    if (normalized === html) {
      // Regex fallback handles simple id selectors.
      for (const selector of this.config.excludeSelectors) {
