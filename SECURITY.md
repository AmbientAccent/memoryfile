...
- Random salt (16 bytes) and IV (12 bytes) generated for each encryption
- Password-based encryption (user provides password when opening/saving)
- Password change capability (re-encrypt with new password on any save)
...
**Security Properties:**
- Brute-force resistance: 100,000 PBKDF2 iterations make password cracking impractical
- Tampering detection: GCM authentication tag prevents unauthorized modifications
- No password storage: Password is never stored in the file, only used for key derivation
- Forward secrecy: Changing password re-encrypts all data with new cryptographic material

**Important Notes:**
- Users must remember their password - there is no password recovery mechanism
- Password strength matters - use strong, unique passwords for sensitive data
- Encryption protects data at rest (in the file) but not in browser memory during use
- For highly regulated data (HIPAA, SOX), ensure passwords meet organizational requirements
